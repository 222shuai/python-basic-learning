	...
	def generate_matrix_dfs(self):
		...
		def dfs(row, col):
			...
			for d in directions:
				...
				if 0 < row_ < self.height - 1 and 0 < col_ < self.width - 1 and visit_flag[row_][
					col_] == 0 and check(col, row_, col_):
					if row == row_:
	...
	def generate_matrix_prim(self):
		...
		def check(rows, cols):
			...
			for lt in [[0, 1], [0, -1], [1, 0], [-1, 0]]:
				temp_sum += self.matrix[rows + lt[0]][cols + lt[1]]
			return temp_sum < -3
		...
		while len(queue) != 0:
			...
			if check(row, col):
				...
				for d in [[0, 2], [0, -2], [2, 0], [-2, 0]]:
					row_, col_ = row + d[0], col + d[1]
					if 0 < row_ < self.height - 1 and 0 < col_ < self.width - 1 and self.matrix[row_][
						col_] == -1:
	...
	def generate_matrix_kruskal(self):
		...
		def check(rows, cols):
			...
			for type_list in [[0, 1], [0, -1], [1, 0], [-1, 0]]:
				row1, col1 = rows + type_list[0], cols + type_list[1]
				if 0 < row1 < self.height - 1 and 0 < col1 < self.width - 1 and self.matrix[
					row1, col1] == -1:
		...
		while union_set.count > 1:
			...
			if len(directions):
				...
				for d in directions:
					row_, col_ = row + d[0], col + d[1]
					if union_set.find((row, col)) == union_set.find((row_, col_)):
						continue
					nodes.add((row, col))
					union_set.count -= 1
					union_set.union((row, col), (row_, col_))
	...
	def find_path_dfs(self, destination):
		...
		def dfs(path):
			...
			for d in [[0, 1], [0, -1], [1, 0], [-1, 0]]:
				...
				if 0 < row_ < self.height - 1 and 0 < col_ < self.width and visited[row_][
